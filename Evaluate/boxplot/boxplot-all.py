...
for file in sys.argv[1:]:
  ...
  for row in csv.DictReader(open(file)):
    if (not graph_name in elapsed):
      elapsed[graph_name] = {}
      timestamps[graph_name] = {}
      starttimes[graph_name] = {}
      errors[graph_name] = {}
    if (not threads in elapsed[graph_name]):
      elapsed[graph_name][threads] = []
      timestamps[graph_name][threads] = []
      starttimes[graph_name][threads] = []
      errors[graph_name][threads] = []
    elapsed[graph_name][threads].append(int(row['elapsed']))
    timestamps[graph_name][threads].append(int(row['timeStamp']))
    starttimes[graph_name][threads].append(int(row['timeStamp']) - int(row['elapsed']))
    if (row['success'] != 'true'):
      errors[graph_name][threads].append(int(row['elapsed']))
