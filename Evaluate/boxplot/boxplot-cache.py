...
for file in sys.argv[1:]:
  # parse cache file
  if ('cache' in file) :
    threads = int(file.split('-')[2].strip('.txt'))
    for row in csv.reader(open(file), delimiter=' '):
      if len(row) < 2 : 
        continue
      if ('get_hits' in row[1]) :
        get_hits = int(row[2])
      if ('get_misses' in row[1]) :
        get_misses = int(row[2])
    # save the percentage to dictionary
    cache.append(round(get_hits/(get_hits+get_misses)*100,2))
...
for label in elapsed:
  ...
  ax3.set_ylim(0,100)
  ax3.plot(cache, 's-', color=palered, linewidth=2, markersize=8)
 
  # Label the axis
  ax1.set_title(label)
  ax1.set_xlabel('Number of concurrent users')
  ax2.set_ylabel('Requests per second')
  ax3.set_ylabel('Percentages of cache hit')
  ax1.set_ylabel('Milliseconds')
  ax1.set_xticks(range(1, len(plot_labels) + 1, 1))
  ax1.set_xticklabels(plot_labels[0::1])
  subplots_adjust(top=0.9, bottom=0.15, right=0.80, left=0.15)
  ...
  line4 = Line2D([], [], marker='s', color=palered, markersize=8, linewidth=2)
  prop = matplotlib.font_manager.FontProperties(size='small')
  figlegend((line1, line2, line3, line4), ('Response Time', 'Throughput', 'Failures (50x)', 'Cache hit'),
    'lower center', prop=prop, ncol=4)
